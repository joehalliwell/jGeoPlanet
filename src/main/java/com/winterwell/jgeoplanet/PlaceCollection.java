...
public class PlaceCollection extends GeoPlanetResource implements Iterable<Place> {
    ...
    public PlaceCollection type(PlaceType... types) throws GeoPlanetException {
        assert types != null;
        if (degree != null)
            throw new UnsupportedOperationException("Cannot specify both type and degree");        // TODO: if types > 1 check we're looking at a places collection
        if (types.length > 7) {
    ...
    public PlaceCollection degree(Integer degree) {
        ...
        if (degree > 1) getClient().log.warn("degree>1 doesn't seem to be working any longer");
